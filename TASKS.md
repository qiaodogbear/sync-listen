...
## 品牌介绍与 GitHub 展示（2026-09-16）

- [x] T701 重写发布会式产品叙事、品牌封面与三步上手，区分已发布和开发预览；同步 GitHub README、About 与 Release 开场，验证渲染、链接和原附件未变。已核对在线正文/Release、26处本地链接、3个下载HTTP200、4附件digest不变；GitHub服务端渲染与封面本地渲染通过，浏览器视觉复核因连接异常未完成。

## 同步与便捷性改进（2026-09-16）

T601-T604 已完成；测试证据见 docs/sync-nearby-validation.md。T605/T606 尚未完成，不计入本轮交付能力。

- [x] T601 同步算法：四时间戳兼容校时、单调时钟、预定位、500ms 检查点、纠偏迟滞与失联退回正常速度。验收：抖动/晚到/重复计划/断线回归通过。
- [x] T602 播放会话独立于页面，增加可关闭的后台播放服务、应用内迷你栏和可选跨应用悬浮窗。验收：权限拒绝不崩溃、成员不可越权、关闭窗口不退出房间、离房清理。
- [x] T603 同一局域网附近用户发现、确认加入与定向邀请弹窗。默认关闭，开启后前台自动发现；会话随机标识、邀请限流/过期、明确确认。验收：协议与安全单测、模拟器 UI、真机 LAN 项单独注明。
- [x] T604 全量回归、更新设计/使用说明和轻量日志，交付调试 APK；不覆盖已发布 v0.3.0。
- [ ] T605 真机声学与不同路由器/热点验收：用双声道录音或共同脉冲测量输出偏差，验证锁屏、蓝牙耳机与 NSD 可达性。受硬件条件限制时保留未完成。
- [ ] T606 后续降低控制等待：实现 PREPARE/READY/COMMIT、单调命令序号、成员就绪/超时策略与按全体成员网络质量自适应的提前量；保留保守回退，依赖 T605 的实测参数。此项尚未实现，本轮仍保留 1500ms 提前量。

## 本轮全量审查与发布（2026-09-16）

- [x] T501 全量文件分块审查与基线验证，证据记录于 `docs/review-2026-09-16.md`。
- [x] T502 修复权限、协议一致性、生命周期、传输和播放中的确认问题并回归。
- [x] T503 完善工程卫生、可重复构建、CI 与正式签名。
- [x] T504 更新 GitHub 介绍、架构图、审查数据展示、使用和发布文档。
- [x] T505 构建产物、模拟运行、安装验收及 SHA-256 校验。
- [x] T506 公开已授权仓库、推送代码并发布可下载 Release。
